...
export async function POST(request: Request) {
  try {
    ...
    await assertWebsitePermission({ userId: user.id, websiteId: input.websiteId, permission: "website:write" });
    const website = await prisma.website.findFirst({ where: { id: input.websiteId }, include: { pages: { take: 1 } } });
    ...
    await audit({ userId: user.id, action: "abtest.create", resource: "ABTest", resourceId: test.id, request, metadata: { websiteId: website.id } });
