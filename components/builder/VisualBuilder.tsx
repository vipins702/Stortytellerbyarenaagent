...
function SectionPreview({ section, update, products }: { section: BuilderSection; update: (id: string, props: Record<string, any>) => void; products: Product[] }) {
  ...
  if (section.type === "scrollStory") return <div className="grid gap-8 bg-charcoal p-10 text-white md:grid-cols-[.9fr_1.1fr]"><div><Badge>Scroll story</Badge><Editable value={p.title || "A journey told in motion"} onSave={(title) => update(section.id, { title })} className="mt-4 font-serif text-5xl font-black tracking-[-.05em]" /><Editable value={p.body || "Guide visitors through a polished sequence of visuals and narrative details."} onSave={(body) => update(section.id, { body })} className="mt-4 leading-8 text-white/65" /><div className="mt-6 grid gap-3">{(p.chapters || []).map((chapter: any, i: number) => <div key={i} className="rounded-2xl border border-white/10 bg-white/10 p-4"><b>{chapter.title}</b><p className="text-sm text-white/60">{chapter.body}</p></div>)}</div></div><div className="sticky top-24 grid h-96 place-items-center overflow-hidden rounded-[2rem] bg-gradient-to-br from-gold/40 to-white/10"><span className="text-7xl">◌</span><p className="absolute bottom-5 text-sm text-white/60">{p.mediaUrl ? "Media connected" : "Upload video or image, then add URL in metadata"}</p></div></div>;
